...
const ItemCell = ({ item }: { item: Product }) => {
  ...
  return (
    ...
    <div className="group flex flex-col overflow-hidden rounded-2xl bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] ring-1 ring-zinc-100 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)]">
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="text-base font-bold text-zinc-900 line-clamp-1" title={item.name}>{item.name}</h3>
        <p className="mt-1 flex-1 text-sm text-zinc-500 line-clamp-2">{item.description}</p>
